  ...
  div.innerHTML = `<button id=run title='Ctrl+Enter'>Run</button>
   ...
   <a href=https://gabrielsroka.github.io/console target=_blank>console v18</a>
...
for await (user of getObjects(url)) {\n
  log(user.id, user.profile.login)\n
   ...
   <textarea id=debug spellcheck=false autocapitalize=none></textarea>
   <div id=results></div>`;
  div.style.cssText = 'position: absolute; padding: 8px; width: 100%; top: 0px; background-color: white; z-index: 1001;';
  const debug = document.getElementById('debug');
  editor.style.cssText = debug.style.cssText = 'width: 100%; height: 300px; font-family: monospace;';
  run.style.cssText = cancelBtn.style.cssText = newBtn.style.cssText = openBtn.style.cssText = saveBtn.style.cssText = closeBtn.style.cssText = 'display: inline-block;';
  run.onclick = runCode;
  ...
  function table(rows, cols, sort) {
    ...
    if (sort) sort.split(',').reverse().forEach(sort => rows.sort(key(sort)));
    if (cols) {
      cols = cols.split(',');
  ...
  async function report(url, cols, name, ui = true, sort) {
    ...
    if (sort) sort.split(',').reverse().forEach(sort => objs.sort(key(sort)));
    if (ui) {
      reportUI(objs, '', name);
    } else {
