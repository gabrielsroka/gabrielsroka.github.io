(function () {
    ...
    // Sous-menu interface users -> a supprimer potentiellement, voir avec Geoffrey
    ...
    //Fonction afficher 10 de plus
    function displayMoreOrLess() {
        var nbAffiches = 10; // Nombre d'éléments initialement affichés
        var items = document.querySelectorAll("#maListe li");
        var totalItems = items.length;
        var bouton = document.getElementById("afficherPlus");
        var isShowingMore = false; // Flag pour suivre si on montre plus d'éléments ou non
        ...
        function toggleAffichage() {
            if (isShowingMore) {
                // Si on montre actuellement plus d'éléments, on réduit à 10
                nbAffiches = 10;
                isShowingMore = false;
            } else {
                // Sinon, on montre tous les éléments (ou 10 de plus si vous préférez)
                nbAffiches = totalItems;
                isShowingMore = nbAffiches < totalItems ? false : true; // Mettre à jour selon que tous les éléments sont montrés ou non
            }
            afficherItems();
        }

        afficherItems(); // Affiche initialement les éléments
    ...
    // Fonction générique pour créer un popup avec barre de recherche
    ...
    // Fonction générique pour effectuer des requêtes AJAX et afficher les résultats
    function fetchDataAndDisplay(type) {
        const url = config.baseUrl;
        const apiToken = config.apiToken; // Assurez-vous de gérer ce token de manière sécurisée
