...
function SimulatorWidget(node) {
  ...
  function UI() {
    ...
    function captureTabInEditor(e) { // no more tabs. now w/ Ctrl+Enter
        const ENTER = 13;
  ...
  function Display() {
    ...
    function updatePixel(addr) {
      ...
      var y = Math.floor((addr - 0x200) / numY);
      var x = (addr - 0x200) % numX;
  ...
  function Simulator() {
    ...
    function runBinary() {
        ...
        ui.play();
        codeRunning = true;
        executeId = setInterval(multiExecute, 1);
      }
    }
    ...
    function reset() {
      display.reset();
      for (var i = 0; i < 0x600; i++) { // clear ZP, stack and screen, TODO
        memory.set(i, 0x00);
      }
      regA = regX = regY = 0;
      regPC = 0x600; // TODO
    ...
    function stop() {
      ...
      message("Stopped. elapsed time: " + (new Date() - startTime));
    }
  ...
  function Assembler() {
    ...
    function assembleCode() {
      var BOOTSTRAP_ADDRESS = 0x600; // TODO
    ...
    function hexdump() {
      openPopup(memory.format(0x600, codeLen), 'Hexdump'); // TODO
    }
